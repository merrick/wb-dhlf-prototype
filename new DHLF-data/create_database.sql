...
-- Script Version: 2.2.7
-- Generated on: 2025-10-15 12:17:04
...
-- Count roles by type  
SELECT role_type, COUNT(*) as role_count
FROM roles GROUP BY role_type;

-- Get competencies required for Digital Health Enterprise Architect
SELECT 
    r.role_title,
    d.domain_title,
    s.subdomain_title,
...
FROM roles r
JOIN role_competencies rc ON r.role_id = rc.role_id
JOIN competencies c ON rc.competency_id = c.competency_id
JOIN subdomains s ON c.subdomain_id = s.subdomain_id
JOIN domains d ON s.domain_id = d.domain_id
WHERE r.role_title = 'Digital Health Enterprise Architect'
ORDER BY d.sort_order, s.sort_order, c.sort_order;

-- Get roles that require a specific competency
SELECT 
    c.competency_code,
    c.competency_title,
    r.role_title,
    r.role_type
FROM competencies c
JOIN role_competencies rc ON c.competency_id = rc.competency_id
JOIN roles r ON rc.role_id = r.role_id
WHERE c.competency_code = '3.1.2'
ORDER BY r.role_title;

-- Count competencies by role
SELECT 
    r.role_title,
    COUNT(rc.competency_id) as competency_count
FROM roles r
LEFT JOIN role_competencies rc ON r.role_id = rc.role_id
